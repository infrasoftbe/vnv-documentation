...
<BrowserOnly>{
  () => {
    ...
    React.useEffect(() => {
      ...
      if(_package)fetch( `http://localhost:8080/schemas/${_package}` )
      .then(( response ) => response.json())
      .then(( result ) => setSchemaList( result ))

    } , [ _package ]);

    React.useEffect(() => {

      if(_package)fetch( `http://localhost:8080/schemas/${_package}/${schema}` )
      .then(( response ) => response.json())
      .then(( result ) => setTargetSchema( result ))
